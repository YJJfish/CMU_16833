...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom):
        ...
        b[(i+1)*2:(i+1)*2+2] += sqrt_inv_odom @ odoms[i, :]
    ...
    for i in range(n_obs):
        ...
        A[(n_odom+1+i)*2:(n_odom+1+i)*2+2, col_idx] += sqrt_inv_obs @ np.array([[-1, 0, 1, 0], [0, -1, 0, 1]], dtype=np.float64)
        b[(n_odom+1+i)*2:(n_odom+1+i)*2+2] += sqrt_inv_obs @ measurement
