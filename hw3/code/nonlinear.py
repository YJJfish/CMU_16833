...
def bearing_range_estimation(x, i, j, n_poses):
    ...
    obs = np.zeros((2, ))
    dx = x[(n_poses+j)*2] - x[i*2]
    dy = x[(n_poses+j)*2+1] - x[i*2+1]
    obs[0] = warp2pi(np.arctan2(dy, dx))
    obs[1] = np.sqrt(dy**2 + dx**2)
    return obs
...
def compute_meas_obs_jacobian(x, i, j, n_poses):
    ...
    deno2 = np.sqrt(deno1)
    jacobian = np.array([
        [dy/deno1, -dx/deno1, -dy/deno1, dx/deno1],
        [-dx/deno2, -dy/deno2, dx/deno2, dy/deno2]
    ], dtype=np.float64)
    return jacobian
...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom):
        A[(i+1)*2:(i+1)*2+2, i*2:i*2+4] += sqrt_inv_odom @ np.array([[-1, 0, 1, 0], [0, -1, 0, 1]], dtype=np.float64)
        b[(i+1)*2:(i+1)*2+2] += sqrt_inv_odom @ (odoms[i, :] - odometry_estimation(x, i)) # This is an iterative process.
    # TODO: Then fill in landmark measurements
    for i in range(n_obs):
        pose_idx = int(observations[i, 0])
        landmark_idx = int(observations[i, 1])
        measurement = observations[i, 2:4]
        col_idx = [2*pose_idx, 2*pose_idx+1, 2*(n_poses+landmark_idx), 2*(n_poses+landmark_idx)+1]
        A[(n_odom+1+i)*2:(n_odom+1+i)*2+2, col_idx] += sqrt_inv_obs @ compute_meas_obs_jacobian(x, pose_idx, landmark_idx, n_poses)
        temp_b = measurement - bearing_range_estimation(x, pose_idx, landmark_idx, n_poses)
        temp_b[0] = warp2pi(temp_b[0])
        b[(n_odom+1+i)*2:(n_odom+1+i)*2+2] += sqrt_inv_obs @ temp_b
