...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        self.points[indices] = (w * self.points[indices] + 1.0 * (R @ points.T + t).T) / (w + 1.0)
        self.normals[indices] = normalized((w * self.normals[indices] + 1.0 * (R @ normals.T).T) / (w + 1.0))
        self.colors[indices] = (w * self.colors[indices] + 1.0 * colors) / (w + 1.0)
        self.weights[indices] = w + 1.0
    ...
    def add(self, points, normals, colors, R, t):
        ...
        self.weights = np.concatenate((self.weights, np.ones((N, 1))))
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        '''
        return (np.linalg.norm(points - input_points, axis=1) < dist_diff) & (np.abs(np.arccos(np.sum(normals * input_normals, axis=1))) < angle_diff)
