...
def find_projective_correspondence(source_points,
    ...
    mask = (np.linalg.norm(p - q, axis=1) < dist_diff)
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    for i in range(M):
        n_q_i = n_q[i] # 3
        p_prime_i = p_prime[i] # 3
        q_i = q[i] # 3
        A_i = np.hstack([np.cross(p_prime_i, n_q_i), n_q_i]).reshape(1, 6) # 1x6
        b_i = n_q_i @ (p_prime_i - q_i) # scalar
        A += A_i.T @ A_i
        b += -A_i.T * b_i
    # End of TODO
